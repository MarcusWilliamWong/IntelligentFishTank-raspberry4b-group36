...
uint8_t VEML7700::sendData(uint8_t command, uint16_t data)
{
...
uint8_t VEML7700::receiveData(uint8_t command, uint16_t& data)
{
...
uint8_t VEML7700::setGain(als_gain_t gain)
{
...
uint8_t VEML7700::getGain(als_gain_t& gain)
{
	gain = als_gain_t(
...
uint8_t VEML7700::setIntegrationTime(als_itime_t itime)
{
...
uint8_t VEML7700::getIntegrationTime(als_itime_t& itime)
{
	itime = als_itime_t(
...
uint8_t VEML7700::setPersistence(als_persist_t persist)
{
...
uint8_t VEML7700::setPowerSavingMode(als_powmode_t powmode)
{
...
uint8_t VEML7700::setALSHighThreshold(uint16_t thresh)
{
	return sendData(COMMAND_ALS_WH, thresh);
}

uint8_t VEML7700::setALSLowThreshold(uint16_t thresh)
{
	return sendData(COMMAND_ALS_WL, thresh);
}

uint8_t VEML7700::getALS(uint16_t& als)
{
	return receiveData(COMMAND_ALS, als);
}

uint8_t VEML7700::getWhite(uint16_t& white)
{
...
void VEML7700::scaleLux(uint16_t raw_counts, float& lux)
{
	als_gain_t gain;
	als_itime_t itime;
	getGain(gain);
